...
#authors:                                                                  #
#                                                                          #
#Ferhat OZOK &          Nasuf SONMEZ          &     Kerem CANKOCAK         #
#ferhat.ozok@cern.ch   Nasuf.sonmez@cern.ch      kerem.cankocak@cern.ch    #
#                                                                          #
#                                                                          #
############################################################################
*/
...
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxx	
  ...
  Nsbsb=0;
  Nstst=0;
  kok1=kok2=kok3=kok4=kok5=kok6=kok7=kok8=kok9=kok10=kok11=kok12=kok13=kok14=kok15=kok16=fsa=fsb=fsc=0;
		...
		for(int j = i+2; j < (int) MCData.size(); j++) {	
			if ( abs(MCData[j]->pid())==1000022&&(MCData[j]->motherIndex()==i
			     ||MCData[j]->motherIndex()==i+1)) {
			       say_0++;
			       if(say_0==2)fsa++;
			       } //2jet eventler icin saydirmak F.O.
	         }
	}
  } 
   ...
   if ((abs(MCData[MCData[i]->motherIndex()]->pid()) < 5) && (abs(MCData[MCData[i]->motherIndex()]->pid()) > 0)){ ++kok5; } 
     if ( abs(MCData[MCData[i]->motherIndex()]->pid()) == 21 ){++kok6; }
		...
		if ( abs(MCData[j]->pid())==1000022 &&((abs(MCData[MCData[j]->motherIndex()]->pid())>1000000
		    ...
		    && abs(MCData[MCData[j]->motherIndex()]->pid())<2000005)&& MCData[MCData[MCData[j]->motherIndex()]->motherIndex()]->motherIndex()==i ||
		 ...
		 if(say_1==2)fsb++;
		  ...
		  //3jet eventler icin saydirmak F.O.
	   }
      ...
      if ( abs(MCData[MCData[i]->motherIndex()]->pid()) == 21 ){++kok10; }
		...
		if ( abs(MCData[j]->pid())==1000022 &&((abs(MCData[MCData[j]->motherIndex()]->pid())>1000000
		 ...
		 if(say_2==2)fsc++;
		  ...
		  //4jet eventler icin saydirmak F.O.
	   }
